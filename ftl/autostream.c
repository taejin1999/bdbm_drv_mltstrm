...
uint64_t num_chunks_in_q[BDBM_STREAM_NUM+1] = {0};
...
uint32_t autostream_destroy_queues(void) {
	...
	/*
	for(qid = BDBM_STREAM_NUM; qid > 0; qid--) {
		list_for_each(pos, &autostream_queue[qid]) {
			chunk = list_entry(pos, autostream_chunk_t, list);
			break;
		}
		bdbm_msg("q: %d, chunks: %lld, top_ref_cnt: %lld", qid, num_chunks_in_q[qid], (chunk==NULL)?-1:chunk->ref_cnt);
	}
	*/
		...
		if(chunk != NULL){
			if(chunk->exp_time < dev_total_ref_cnt) {
				if(chunk->cid == hottest_chunk->cid) {
					...
					hottest_chunk = find_next_hottest_chunk(hottest_chunk->cid);
					if(hottest_chunk == NULL) {
				...
				num_chunks_in_q[chunk->qid]--;
				list_del(&chunk->list);
				chunk->qid = qid-1;
				bdbm_bug_on(chunk->qid == 0);
				list_add_tail(&chunk->list, &autostream_queue[qid-1]);
				num_chunks_in_q[qid-1]++;
				/*
...
int32_t get_queue_index(uint64_t sLBA) {
